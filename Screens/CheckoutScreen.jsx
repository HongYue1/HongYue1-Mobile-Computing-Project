  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      if (user && user.isAuthenticated && user.uid) {
        ...
        const docRef = await addDoc(collection(db, "orders", user.uid, "orders"), orderData);
      } else {
        console.warn("User not authenticated or missing UID, order not placed.", user);
      }
      clearCart();
      ...
      showAlert(
        "Order Failed",
        "Could not place your order. Please try again. Details: " + error.message
      );
    } finally {
      setLoading(false);
    }
